...
def PILtoTorch(pil_image, resolution):
    """
    Convert PIL image to torch tensor with safe alpha handling to avoid
    black splat artifacts in 3DGS.
    """
    if pil_image.mode == 'RGBA':
        image_np = np.array(pil_image).astype(np.float32)
        rgb_np   = image_np[..., :3] / 255.0
        alpha_np = image_np[..., 3:] / 255.0

        # --- Fix for black splats ---
        # Threshold alpha to reduce floaters
        alpha_np = (alpha_np > 0.5).astype(np.float32)

        # Where alpha=0, set RGB to white (or background)
        rgb_np[alpha_np.squeeze() < 0.5] = 1.0

        pil_image = Image.fromarray((rgb_np * 255).astype(np.uint8))

    resized_image_PIL = pil_image.resize(resolution, Image.LANCZOS)
    resized_image = torch.from_numpy(np.array(resized_image_PIL).astype(np.float32)) / 255.0

    if len(resized_image.shape) == 3:
        return resized_image.permute(2, 0, 1)  # C, H, W
    else:
        return resized_image.unsqueeze(dim=-1).permute(2, 0, 1)
