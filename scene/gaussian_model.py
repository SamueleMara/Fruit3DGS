...
class GaussianModel:
    ...
    def restore(self, model_args, training_args):
        ...
        opt_dict,
        self.spatial_lr_scale,
        semantic_mask) = model_args  # ADDED semantic_mask

        self.training_setup(training_args)
        self.xyz_gradient_accum = xyz_gradient_accum
        self.denom = denom
        # load semantic mask if present in checkpoint
        if semantic_mask is not None:
            # ensure it's a parameter
            self.semantic_mask = nn.Parameter(semantic_mask.requires_grad_(True))
    ...
    def training_setup(self, training_args):
        ...
        # build parameter groups; include semantic_mask here if present BEFORE creating optimizer
        l = [
    ...
    def save_ply(self, path):
        ...
        # assemble attributes in same order as construct_list_of_attributes()
    ...
    def load_ply(self, path, use_train_test_exp = False):
        ...
        # --- load optional semantics if present in PLY ---
        if 'semantics' in plydata.elements[0].data.dtype.names:
            semantics_arr = np.asarray(plydata.elements[0]['semantics'])
            self.semantic_mask = nn.Parameter(
                torch.tensor(semantics_arr, dtype=torch.float32, device="cuda").requires_grad_(True)
            )
    ...
    def prune_points(self, mask):
        ...
        # tmp_radii may not exist before densification; handle safely
        if hasattr(self, "tmp_radii"):
            self.tmp_radii = self.tmp_radii[valid_points_mask]
        if self.semantic_mask is not None:
            self.semantic_mask = self.semantic_mask[valid_points_mask]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_tmp_radii, new_semantic_mask=None):
        d = {"xyz": new_xyz,
        ...
        if "semantic_mask" in optimizable_tensors:
            self.semantic_mask = optimizable_tensors["semantic_mask"]

        # tmp_radii might not exist if not supplied earlier; ensure attribute creation
        if hasattr(self, "tmp_radii"):
            self.tmp_radii = torch.cat((self.tmp_radii, new_tmp_radii))
        else:
            self.tmp_radii = new_tmp_radii.clone()
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        # New semantic extension: if semantic_mask present, clone the selected semantic entries
        new_semantic_mask = None
        if self.semantic_mask is not None:
            new_semantic_mask = self.semantic_mask[selected_pts_mask].repeat(N)

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest,
                           new_opacities, new_scaling, new_rotation,
                           new_tmp_radii, new_semantic_mask=new_semantic_mask)

        prune_filter = torch.cat((selected_pts_mask, torch.zeros(N * int(selected_pts_mask.sum().item()), device="cuda", dtype=bool)))
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_tmp_radii = self.tmp_radii[selected_pts_mask] if hasattr(self, "tmp_radii") else torch.zeros((new_xyz.shape[0],), device="cuda")

        new_semantic_mask = None
        if self.semantic_mask is not None:
            new_semantic_mask = self.semantic_mask[selected_pts_mask]
